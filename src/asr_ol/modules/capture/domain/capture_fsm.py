"""Wake-triggered one-sentence capture state machine."""
...
class CaptureState(str, Enum):
    """Finite states for one-shot capture orchestration."""
...
class _CaptureSession:
    """Mutable in-flight capture session state."""
...
class CaptureWindow:
    """Finalized time window used to extract capture transcript."""
...
class CaptureFSM:
    ...
    def __init__(self, pre_roll_ms: int, armed_timeout_ms: int) -> None:
        """Create FSM timing configuration."""
    ...
    def on_vad(self, event: VadEvent) -> CaptureWindow | None:
        """Advance FSM on VAD boundary and emit finalized window when complete."""
        now = event.ts
    ...
    def tick(self, now: float | None = None) -> None:
        """Advance timeout checks without a new event."""
